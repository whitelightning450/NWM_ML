...
def TS_plot(dictionary, model, path, title, freq, supply = False):
    ...
    keys_name = list(temp_dictionary.keys())
    
    # Initialize a list to store the sliced results
    dictionary = []
    number = 1
    for index_number in range(0, len(temp_dictionary), 9):
        dictionary = {key: temp_dictionary[key] for key in keys_name[index_number:index_number+9]}
        ...
        #make figure
        if len(keys) <= 3:
            fig, ax = plt.subplots(1,len(keys), figsize=(10, 3), gridspec_kw={'hspace': 0.4, 'wspace': 0.4})

        if 3 < len(keys) <= 6:
            fig, ax = plt.subplots(2,len(keys), figsize=(10, 6), gridspec_kw={'hspace': 0.4, 'wspace': 0.4})
        ...
        plt.savefig(path+f"_{number}.png", dpi = 600, bbox_inches = 'tight')
        plt.show()
        number += 1
