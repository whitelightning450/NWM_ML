...
class CustomCNN1D(nn.Module):
    def __init__(self, channels, kernel_sizes, layer_sizes, device=None):
        ...
        for i in range(len(kernel_sizes)):
            self.conv_layers.append(nn.Conv1d(channels[i], channels[i + 1], kernel_sizes[i]))
            self.conv_layers.append(nn.ReLU())
            self.conv_layers.append(nn.MaxPool1d(kernel_size=kernel_sizes[i]))
